      ...
      {expanded && (
        <View>
          ...
          <View style={card.balRow}>
            {[
              { l: translate("retailer_remain_pre"),  v: `₹ ${item.retailer_remain_pre ?? "0"}`,  c: "#1D4ED8", bg: "#EFF6FF" },
              { l: translate("network"),               v: `₹ ${item.network ?? "0"}`,               c: "#D97706", bg: "#FEF3C7" },
              { l: translate("retailer_remain_post"), v: `₹ ${item.retailer_remain_post ?? "0"}`, c: "#16A34A", bg: "#DCFCE7" },
            ].map((b) => (
          ...
          <View style={card.balRow}>
            {[
              { l: translate("Retailer_gst"), v: `₹ ${item.Retailer_gst ?? "0"}`, c: "#7C3AED", bg: "#F5F3FF" },
              { l: translate("Retailer_tds"), v: `₹ ${item.Retailer_tds ?? "0"}`, c: "#DC2626", bg: "#FEE2E2" },
...
const PanReport = () => {
  ...
  const recentTransactions = useCallback(async (
    ...
    try {
      ...
      const url  = `${APP_URLS.PanReport}ddl_status=${status}&txt_frm_date=${fmt(from)}&txt_to_date=${fmt(to)}`;
      const url2 = `${APP_URLS.dealer_Rem_Pancard_report_new}ddl_status&txt_frm_date=${fmt(from)}&txt_to_date=${fmt(to)}&allretailer=${retailerId}`;
      const response = await get({ url: IsDealer ? url2 : url });
      const data = response.Message || [];
     setTransactions(IsDealer ? (Array.isArray(response) ? response : []) : data);
    } catch (error) {
            ...
            ListFooterComponent={
              transactions.length > present ? (
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}
                >
                  <Text style={styles.loadMoreTxt}>{translate("Load More")}</Text>
                </TouchableOpacity>
              ) : null
            }
          />
