...
const FundTransferRetailer = () => {
    ...
    const UniqueId = async () => {
        ...
        console.log([amount, retailerdata?.UserID, paymentMode, Pin, transactionn, collectionBy, comment]);
    ...
    const submittxn = async () => {
        ...
        if (!amount || !retailerdata?.UserID || !paymentMode || !Pin || !txnId) {
            alert('Please fill in all required fields.');setIsLoading(false)
            return;
        }
    ...
    const submittxnWALLET = async () => {
        ...
        if (!amount || !retailerdata?.UserID || !paymentMode || !Pin || !transaction || !Wallet || !Walletn || !paymenttype) {
            alert('Please fill in all required fields.');setIsLoading(false)
...
console.log(!retailerdata?.UserID || !paymentMode  || !Subject || !comment)
        ...
        const encryption = await encrypt([retailerdata?.UserID, paymentMode, Pin, transaction, Subject, comment]);
        console.log([amount, retailerdata?.UserID, paymentMode, Pin, transaction, collectionBy, comment]);
        console.log('***********************');
...
{isLoading&&<ShowLoader/>}
                <DynamicButton
                    title={paymentMode == 'Charge Back' && cbStatus ? 'Get Otp' : (isSubmit ? 'Confirm Transfer' : 'Submit')}
                    onPress={() => {
                        console.log(userId)
            ...
            <BottomSheet
        animationType="none"   isVisible={isVisible}>
                <View style={styles.bottomsheetview}>
                    ...
                    {/* Search Bar */}
                    <TextInput
                        placeholder={ !retailerMode ? "Search by Name /Firm name/ Mobile...":'Search'}
                        value={searchQuery}
            ...
            <BottomSheet
        animationType="none"   isVisible={admin}>
                <View style={styles.bottomsheetview}>
                    <View style={[styles.StateTitle, { backgroundColor: '#A870B7' }]}>
                        <View style={styles.titleview}>
                            <Text style={styles.stateTitletext}>
            ...
            <BottomSheet
        animationType="none"   isVisible={Type}>
                <View style={styles.bottomsheetview}>
                    <View style={[styles.StateTitle, { backgroundColor: '#A870B7' }]}>
                        <View style={styles.titleview}>
                            <Text style={styles.stateTitletext}>
